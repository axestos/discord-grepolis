...
const grepolis_get_requests_1 = require("../grepolis-get-requests");
var AsciiTable = require('ascii-table');
async function getCityOfPlayer(command, client) {
    ...
    var players = await grepolis_get_requests_1.getPlayers();
    ...
    var towns = await grepolis_get_requests_1.getTowns();
    var townsOfPlayer = towns.filter((town) => { return town.player_id === selectedPlayer.id; });
    townsOfPlayer = townsOfPlayer.sort((aTown, bTown) => {
        if (aTown.name < bTown.name) {
            return -1;
        }
        if (aTown.name > bTown.name) {
            return 1;
...
exports.getCityOfPlayer = getCityOfPlayer;
//# sourceMappingURL=city-of-player-command.js.map